...
describe('User visits addin', () => {
   ...
   // Confirm page displaying after initialized and focus is called
    it('should display root div', async () => {
        ...
        await page.waitFor('#mygeotabSygicPage', {
            visible: true
        });
    ...
    it('blur button should blur addin', async () => {
        ...
        let hidden = await page.evaluate( () => {
            let toggled = false;
            let addin = document.getElementById('mygeotabSygicPage');
    ...
    it('should refocus after group filter is selected', async () => {
        let originalFocus = await page.evaluate( () => geotab.addin.mygeotabSygicPage.focus);

        await page.click('#group-toggle-button');
        await page.evaluate( () => {
            geotab.addin.mygeotabSygicPage.count = 0;
            geotab.addin.mygeotabSygicPage.focus = () => { geotab.addin.mygeotabSygicPage.count++; };
        });
        await page.click('#group-item-b4');

        let invocations = await page.evaluate( () => {
            return geotab.addin.mygeotabSygicPage.count;
        });
        ...
        await page.evaluate( (addinFocus) => {
            geotab.addin.mygeotabSygicPage.focus = addinFocus;
        }, originalFocus);
    });
